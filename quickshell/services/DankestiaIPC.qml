...
Singleton {
    id: root
    readonly property var log: Log.scoped("DankestiaIPC")

    property bool dankestiaAvailable: false
    ...
    readonly property string socketPath: Quickshell.env("DANKESTIA_SOCKET")
    ...
    signal sysinfoStateUpdate(var data)

    property bool capsLockState: false
    property bool screensaverInhibited: false
    property var screensaverInhibitors: []

    property var activeSubscriptions: ["compositor", "network", "network.credentials", "loginctl", "freedesktop", "freedesktop.screensaver", "gamma", "theme.auto", "bluetooth", "bluetooth.pairing", "dwl", "brightness", "wlroutput", "evdev", "browser", "dbus", "clipboard", "location", "sysupdate", "sysinfo"]
    ...
    Process {
        ...
        onExited: exitCode => {
            if (exitCode === 0) {
                root.dankestiaAvailable = true;
                connectSocket();
            } else {
                root.dankestiaAvailable = false;
            }
        }
    }

    function connectSocket() {
        if (!dankestiaAvailable || isConnected || isConnecting) {
            return;
        }
    ...
    function handleSubscriptionEvent(response) {
        ...
        if (service === "server") {
            ...
            sysinfoStateUpdate(data);
        }
    }

    function sendRequest(method, params, callback) {
        if (!isConnected) {
            log.warn("DankestiaIPC.sendRequest: Not connected, method:", method);
            if (callback) {
                callback({
        ...
        if (method.startsWith("clipboard")) {
            ...
            log.debug("DankestiaIPC.sendRequest: Sending request id=" + id + " method=" + method);
        }
