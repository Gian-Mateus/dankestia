#!/bin/bash
# Unified OBS upload script for dankestia packages
...
#
# Examples:
#   ./distro/scripts/obs-upload.sh dankestia "Update to v1.0.2"
#   ./distro/scripts/obs-upload.sh debian dankestia
#   ./distro/scripts/obs-upload.sh opensuse dankestia-git
#   ./distro/scripts/obs-upload.sh debian dankestia-git 2    # Rebuild with db2 suffix
#   ./distro/scripts/obs-upload.sh dankestia-git --rebuild=2 # Rebuild with db2 suffix (flag syntax)
...
if [[ -z "$PACKAGE" ]]; then
    ...
    echo ""
    echo "  1. dankestia         - Stable DANKESTIA"
    echo "  2. dankestia-git     - Nightly DANKESTIA"
    echo "  3. dankestia-greeter - DANKESTIA greeter for greetd"
    echo "  a. all"
    echo ""
...
# Bundled Go for dankestia-git OBS builds (offline VM); filenames must match distro/opensuse/dankestia-git.spec Source1/2.
GO_TOOLCHAIN_CACHE="${GO_TOOLCHAIN_CACHE:-$HOME/.cache/dankestia-obs-go-toolchain}"

dankestia_git_go_toolchain_version() {
    grep -m1 '^go ' "$REPO_ROOT/core/go.mod" 2>/dev/null | awk '{print $2}'
}

ensure_dankestia_git_go_tarballs() {
    local dest="$1"
    local ver arch url cached
    ver="$(dankestia_git_go_toolchain_version)"
    if [[ -z "$ver" ]]; then
...
update_debian_dankestia_service() {
    ...
    sed -i "s|/releases/download/v[0-9][^\"]*/dankestia-distropkg-amd64\.gz|/releases/download/v${base_version}/dankestia-distropkg-amd64.gz|" "$service_path"
    sed -i "s|/releases/download/v[0-9][^\"]*/dankestia-distropkg-arm64\.gz|/releases/download/v${base_version}/dankestia-distropkg-arm64.gz|" "$service_path"
}

update_debian_dankestia_greeter_service() {
    local service_path="$1"
    if [[ -z "$service_path" || ! -f "$service_path" ]]; then
        return 0
    fi
    ...
    sed -i "s|/releases/download/v[0-9][^\"]*/dankestia-qml\.tar\.gz|/releases/download/v${base_version}/dankestia-qml.tar.gz|" "$service_path"
}
...
update_opensuse_git_spec() {
    ...
    go_ver="$(dankestia_git_go_toolchain_version)"
...
if [[ -d "distro/debian/$PACKAGE/debian" ]]; then
    ...
    if [[ "$PACKAGE" == *"-git" ]]; then
        ...
        if [[ -z "$BASE_VERSION" ]]; then
            BASE_VERSION=$(grep -oP '^Version:\s+\K[0-9.]+' distro/opensuse/dankestia.spec | head -1 || echo "1.0.2")
        fi
    ...
    if [[ "$PACKAGE" == "dankestia" ]] && [[ -f "distro/debian/$PACKAGE/_service" ]]; then
        update_debian_dankestia_service "distro/debian/$PACKAGE/_service"
    elif [[ "$PACKAGE" == "dankestia-greeter" ]] && [[ -f "distro/debian/$PACKAGE/_service" ]]; then
        update_debian_dankestia_greeter_service "distro/debian/$PACKAGE/_service"
    fi
...
if [[ "$UPLOAD_OPENSUSE" == true ]] && [[ -f "distro/opensuse/$PACKAGE.spec" ]]; then
    ...
    if [[ "$PACKAGE" == *"-git" ]]; then
    ...
    elif [[ "$PACKAGE" == "dankestia-greeter" ]] && [[ -n "$CHANGELOG_VERSION" ]]; then
        DANKESTIA_GREETER_BASE_VERSION=$(echo "$CHANGELOG_VERSION" | sed -E 's/^([0-9]+(\.[0-9]+)*).*/\1/')
        DANKESTIA_GREETER_RELEASE=$(echo "$CHANGELOG_VERSION" | sed -E 's/.*db([0-9]+)$/\1/' || echo "1")
        CHANGELOG_DATE=$(date '+%a %b %d %Y')
        sed -i "s/VERSION_PLACEHOLDER/${DANKESTIA_GREETER_BASE_VERSION}/g" "$WORK_DIR/$PACKAGE.spec"
        sed -i "s/RELEASE_PLACEHOLDER/${DANKESTIA_GREETER_RELEASE}/g" "$WORK_DIR/$PACKAGE.spec"
        ...
        sed -i "s/^Version:.*/Version:        ${DANKESTIA_GREETER_BASE_VERSION}/" "$WORK_DIR/$PACKAGE.spec"
        sed -i "s/^Release:.*/Release:        ${DANKESTIA_GREETER_RELEASE}%{?dist}/" "$WORK_DIR/$PACKAGE.spec"
    fi
...
if [[ "$UPLOAD_OPENSUSE" == true ]] && [[ "$UPLOAD_DEBIAN" == false ]] && [[ -f "distro/opensuse/$PACKAGE.spec" ]]; then
    ...
    if [[ -f "distro/debian/$PACKAGE/_service" ]] && grep -q "tar_scm" "distro/debian/$PACKAGE/_service"; then
        ...
        if [[ -n "$GIT_URL" ]]; then
            ...
            SOURCE_DIR="$TEMP_DIR/dankestia-git-source"
    ...
    # For dankestia-greeter: download dankestia-qml.tar.gz from _service URL
    if [[ -z "${SOURCE_DIR:-}" ]] && [[ "$PACKAGE" == "dankestia-greeter" ]] && [[ -f "distro/debian/$PACKAGE/_service" ]]; then
        DANKESTIA_GREETER_URL=$(grep -A 5 'name="download_url"' "distro/debian/$PACKAGE/_service" | grep "path" | sed 's/.*<param name="path">\(.*\)<\/param>.*/\1/' | head -1)
        if [[ -n "$DANKESTIA_GREETER_URL" ]]; then
            DANKESTIA_GREETER_FULL_URL="https://github.com${DANKESTIA_GREETER_URL}"
            echo "    Downloading dankestia-greeter source from: $DANKESTIA_GREETER_FULL_URL"
            if wget -q -O "$TEMP_DIR/dankestia-qml.tar.gz" "$DANKESTIA_GREETER_FULL_URL" 2>/dev/null || \
                curl -L -f -s -o "$TEMP_DIR/dankestia-qml.tar.gz" "$DANKESTIA_GREETER_FULL_URL" 2>/dev/null; then
                cd "$TEMP_DIR"
                tar -xzf dankestia-qml.tar.gz
                if [[ -f "Modules/Greetd/assets/dankestia-greeter" ]]; then
                    SOURCE_DIR="$TEMP_DIR"
                fi
                cd "$REPO_ROOT"
            fi
    ...
    if [[ -n "$SOURCE_DIR" && -d "$SOURCE_DIR" ]]; then
        ...
        if [[ -n "$SOURCE0" ]]; then
            ...
            dankestia-greeter)
                ...
                ;;
            dankestia)
                DANKESTIA_VERSION=$(grep "^Version:" "$REPO_ROOT/distro/opensuse/$PACKAGE.spec" | sed 's/^Version:[[:space:]]*//' | head -1)
                EXPECTED_DIR="Dankestia-${DANKESTIA_VERSION}"
                ;;
            dankestia-git)
                EXPECTED_DIR="dankestia-git-source"
                ;;
            *)
            ...
            if [[ "$PACKAGE" == "dankestia-git" ]]; then
                echo "  - Staging bundled Go toolchains for RPM (Source1/Source2)"
                ensure_dankestia_git_go_tarballs "$WORK_DIR"
            fi
        fi
    else
...
if [[ "$UPLOAD_DEBIAN" == true ]] && [[ -d "distro/debian/$PACKAGE/debian" ]]; then
    ...
    if [[ "$SOURCE_FORMAT" == *"native"* ]]; then
        ...
        if [[ -f "distro/debian/$PACKAGE/_service" ]]; then
            if grep -q "tar_scm" "distro/debian/$PACKAGE/_service"; then
            ...
            elif grep -q "download_url" "distro/debian/$PACKAGE/_service" && [[ "$PACKAGE" != "dankestia-git" ]]; then
                ...
                if [[ -n "$URL_PROTOCOL" && -n "$URL_HOST" && -n "$URL_PATH" ]]; then
                    ...
                    if wget -q -O "$TEMP_DIR/source-archive" "$SOURCE_URL" 2>/dev/null ||
                        ...
                        SOURCE_DIR=$(find . -maxdepth 1 -type d -name "Dankestia-*" | head -1)
                        if [[ -z "$SOURCE_DIR" ]]; then
                            SOURCE_DIR=$(find . -maxdepth 1 -type d ! -name "." | head -1)
                        fi
                        # dankestia-qml.tar.gz extracts flat (quickshell contents, no top-level dir)
                        # Create dankestia-qml wrapper so combined tarball has correct top-level dir (like dankestia)
                        if [[ "$PACKAGE" == "dankestia-greeter" ]] && [[ -f "Modules/Greetd/assets/dankestia-greeter" ]]; then
                            mkdir -p dankestia-qml
                            for f in *; do
                                if [[ -e "$f" && "$f" != "dankestia-qml" && "$f" != "source-archive" ]]; then
                                    mv "$f" dankestia-qml/ 2>/dev/null || true
                                fi
                            done
                            SOURCE_DIR="dankestia-qml"
                        fi
        ...
        # Vendor Go dependencies for dankestia-git
        if [[ "$PACKAGE" == "dankestia-git" ]] && [[ -d "$SOURCE_DIR/core" ]]; then
        ...
        if [[ "$UPLOAD_OPENSUSE" == true ]] && [[ -f "distro/opensuse/$PACKAGE.spec" ]]; then
            ...
            SOURCE0=$(grep "^Source0:" "distro/opensuse/$PACKAGE.spec" | awk '{print $2}' | head -1)
            if [[ -z "$SOURCE0" && "$PACKAGE" == "dankestia-git" ]]; then
        ...
        if [[ "$UPLOAD_DEBIAN" == true ]]; then
            ...
            # For dankestia, rename directory to match what debian/rules expects
            ...
            if [[ "$PACKAGE" == "dankestia" ]]; then
                ...
                if [[ -f "$CHANGELOG_IN_SOURCE" ]]; then
                    ...
                    EXPECTED_DIR="Dankestia-${ACTUAL_VERSION}"
            ...
            if [[ "$PACKAGE" == "dankestia-git" ]]; then
                ...
                ensure_dankestia_git_go_tarballs "$SOURCE_DIR"
            fi
            ...
            if [[ "$PACKAGE" == "dankestia" ]]; then
                ...
                EXPECTED_TARBALL_DIR="Dankestia-${VERSION}"
...
EOF
        ...
        if [[ "$UPLOAD_DEBIAN" == true ]]; then
            ...
            # For dankestia-greeter: create orig tarball so Debian build gets upstream (OBS only passes .dsc Files to Debian)
            DSC_FILES_DEBIAN=""
            if [[ "$PACKAGE" == "dankestia-greeter" ]]; then
                ...
                if [[ -f "distro/debian/$PACKAGE/_service" ]] && grep -q "download_url" "distro/debian/$PACKAGE/_service"; then
                    DG_TEMP=$(mktemp -d)
                    DANKESTIA_GREETER_PATH=$(grep -A 5 'name="download_url"' "distro/debian/$PACKAGE/_service" | grep "path" | sed 's/.*<param name="path">\(.*\)<\/param>.*/\1/' | head -1)
                    if [[ -n "$DANKESTIA_GREETER_PATH" ]]; then
                        DG_URL="https://github.com${DANKESTIA_GREETER_PATH}"
                        echo "  - Downloading dankestia-greeter source for orig tarball: $DG_URL"
                        if wget -q -O "$DG_TEMP/dankestia-qml.tar.gz" "$DG_URL" 2>/dev/null || curl -L -f -s -o "$DG_TEMP/dankestia-qml.tar.gz" "$DG_URL" 2>/dev/null; then
                            ( cd "$DG_TEMP" && tar --no-same-owner -xzf dankestia-qml.tar.gz && mkdir -p "$ORIG_DIR" && \
                              for f in *; do [[ "$f" != "dankestia-qml.tar.gz" && "$f" != "$ORIG_DIR" ]] && mv "$f" "$ORIG_DIR/"; done )
...
if [[ -n "$OBS_FILES" ]]; then
    ...
    for old_file in $(echo "$OBS_FILES" | grep -oP '(?<=name=")[^"]*\.(tar\.gz|tar\.xz|tar\.bz2)(?=")' || true); do
        ...
        # Keep pinned Go toolchain archives (bundled for dankestia-git offline builds)
        if [[ "$old_file" =~ ^go[0-9].+\.linux-(amd64|arm64)\.tar\.gz$ ]]; then
            echo "  - Keeping Go toolchain tarball: $old_file"
            continue
        fi

        # Keep current orig tarball for dankestia-greeter (Debian 3.0 quilt needs it)
        UPSTREAM_VER_CLEAN=$(echo "$CHANGELOG_VERSION" | sed 's/-[^-]*$//' 2>/dev/null)
        if [[ "$PACKAGE" == "dankestia-greeter" ]] && [[ "$old_file" == "${PACKAGE}_${UPSTREAM_VER_CLEAN}.orig.tar.gz" ]]; then
