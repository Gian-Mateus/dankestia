#!/bin/bash
# Unified PPA status checker for DANKESTIA packages
...
#   ./distro/scripts/ppa-status.sh dankestia          # Check dankestia package
#   ./distro/scripts/ppa-status.sh all dankestia-git  # Check all packages in dankestia-git PPA
...
if [[ -n "$PACKAGE_INPUT" ]] && [[ "$PACKAGE_INPUT" != "all" ]]; then
    ...
    PACKAGES=("${ALL_PACKAGES[@]}")
    PPAS=("dankestia" "dankestia-git" "danklinux")
fi
