#!/usr/bin/env bash
# Build a DANKESTIA per-series upload plan by comparing Git/GitHub with Launchpad.
...
PACKAGE_FILTER="dankestia-git"
REBUILD_RELEASE=""
JSON=false

PACKAGES=(
    "dankestia:dankestia:release"
    "dankestia-git:dankestia-git:git"
    "dankestia-greeter:danklinux:release"
)
...
latest_tag() {
    git ls-remote --tags --refs --sort='-v:refname' https://github.com/AvengeMedia/Dankestia.git |
        sed -n '1s|.*/v\{0,1\}||p'
}
