# Spec for DANKESTIA Greeter - OpenSUSE/OBS

%global debug_package %{nil}
%global version VERSION_PLACEHOLDER
%global pkg_summary Dankestia greeter for greetd

Name:           dankestia-greeter
Version:        %{version}
...
%description
Dankestia greeter for greetd login manager. A modern, Material Design 3
...
install -dm755 %{buildroot}%{_datadir}/quickshell/dankestia-greeter
cp -r %{_builddir}/dankestia-qml/* %{buildroot}%{_datadir}/quickshell/dankestia-greeter/

install -Dm755 %{_builddir}/dankestia-qml/Modules/Greetd/assets/dankestia-greeter %{buildroot}%{_bindir}/dankestia-greeter

install -Dm644 %{_builddir}/dankestia-qml/Modules/Greetd/README.md %{buildroot}%{_docdir}/dankestia-greeter/README.md

install -Dpm0644 %{_builddir}/dankestia-qml/systemd/tmpfiles-dankestia-greeter.conf %{buildroot}%{_tmpfilesdir}/dankestia-greeter.conf

install -Dm644 %{_builddir}/dankestia-qml/systemd/sysusers-dankestia-greeter.conf %{buildroot}%{_sysusersdir}/dankestia-greeter.conf

install -Dm644 %{_builddir}/dankestia-qml/LICENSE %{buildroot}%{_docdir}/dankestia-greeter/LICENSE

install -dm755 %{buildroot}%{_sharedstatedir}/greeter

# Remove build and development files
rm -rf %{buildroot}%{_datadir}/quickshell/dankestia-greeter/.git*
rm -f %{buildroot}%{_datadir}/quickshell/dankestia-greeter/.gitignore
rm -rf %{buildroot}%{_datadir}/quickshell/dankestia-greeter/.github
rm -rf %{buildroot}%{_datadir}/quickshell/dankestia-greeter/distro

%posttrans
if [ -d "%{_sysconfdir}/xdg/quickshell/dankestia-greeter" ]; then
    rmdir "%{_sysconfdir}/xdg/quickshell/dankestia-greeter" 2>/dev/null || true
    rmdir "%{_sysconfdir}/xdg/quickshell" 2>/dev/null || true
    rmdir "%{_sysconfdir}/xdg" 2>/dev/null || true
fi

%files
%dir %{_docdir}/dankestia-greeter
%license %{_docdir}/dankestia-greeter/LICENSE
%doc %{_docdir}/dankestia-greeter/README.md
%{_bindir}/dankestia-greeter
%dir %{_datadir}/quickshell
%{_datadir}/quickshell/dankestia-greeter/
%{_tmpfilesdir}/%{name}.conf
%{_sysusersdir}/dankestia-greeter.conf
...
if [ -x /usr/sbin/semanage ] && [ -x /usr/sbin/restorecon ]; then
    semanage fcontext -a -t bin_t '%{_bindir}/dankestia-greeter' >/dev/null 2>&1 || true
    restorecon %{_bindir}/dankestia-greeter >/dev/null 2>&1 || true
    ...
    semanage fcontext -a -t cache_home_t '%{_localstatedir}/cache/dankestia-greeter(/.*)?' >/dev/null 2>&1 || true
    restorecon -R %{_localstatedir}/cache/dankestia-greeter >/dev/null 2>&1 || true
    semanage fcontext -a -t usr_t '%{_datadir}/quickshell/dankestia-greeter(/.*)?' >/dev/null 2>&1 || true
    restorecon -R %{_datadir}/quickshell/dankestia-greeter >/dev/null 2>&1 || true
...
chown -R "$GREETER_USER:$GREETER_GROUP" %{_localstatedir}/cache/dankestia-greeter 2>/dev/null || true
...
if [ ! -f "$PAM_CONFIG" ]; then
    ...
    if [ "$NEEDS_PAM_UPDATE" -eq 1 ]; then
        cp "$PAM_CONFIG" "$PAM_CONFIG.backup-dankestia-greeter"
        write_greetd_pam_config
        chmod 644 "$PAM_CONFIG"
        [ "$1" -eq 1 ] && echo "Updated PAM configuration (old config backed up to $PAM_CONFIG.backup-dankestia-greeter)"
    fi
fi
...
command = "/usr/bin/dankestia-greeter --command COMPOSITOR_PLACEHOLDER"
GREETD_EOF
...
elif ! grep -q "dankestia-greeter" "$GREETD_CONFIG"; then
    if [ -n "$COMPOSITOR" ]; then
        BACKUP_FILE="${GREETD_CONFIG}.backup-$(date +%%Y%%m%%d-%%H%%M%%S)"
        cp "$GREETD_CONFIG" "$BACKUP_FILE" 2>/dev/null || true
        sed -i "/^\[default_session\]/,/^\[/ s|^command =.*|command = \"/usr/bin/dankestia-greeter --command $COMPOSITOR\"|" "$GREETD_CONFIG"
        ...
        CONFIG_STATUS="Skipped dankestia-greeter command update (no supported compositor detected)"
    fi
fi
...
=========================================================================
        DANKESTIA Greeter Installation Complete!
=========================================================================
...
cat << 'EOF'
    ...
    ✓ Greeter directories: /var/cache/dankestia-greeter, /var/lib/greeter
    ✓ SELinux contexts: Applied (if applicable)

Next steps:

1. Enable the greeter:
     dankestia greeter enable

2. Sync your theme with the greeter (optional):
     dankestia greeter sync

3. Check your setup:
     dankestia greeter status
...
if [ "$1" -eq 0 ] && [ -x /usr/sbin/semanage ]; then
    semanage fcontext -d '%{_bindir}/dankestia-greeter' 2>/dev/null || true
    semanage fcontext -d '%{_sharedstatedir}/greeter(/.*)?' 2>/dev/null || true
    semanage fcontext -d '%{_localstatedir}/cache/dankestia-greeter(/.*)?' 2>/dev/null || true
    semanage fcontext -d '%{_datadir}/quickshell/dankestia-greeter(/.*)?' 2>/dev/null || true
fi
