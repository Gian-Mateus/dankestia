---
name: dankestia-plugin-dev
description: >
  Develop plugins for Dankestia (DANKESTIA), a QML-based Linux desktop shell built on
  ...
  to create, modify, or debug a DANKESTIA plugin, or asks about the DANKESTIA plugin API.
...
# Dankestia Plugin Development

## Overview

DANKESTIA plugins extend the desktop shell with custom widgets, background services, launcher
integrations, and desktop widgets. Plugins are QML components discovered from
`~/.config/Dankestia/plugins/`.

**Minimum plugin structure:**

```
~/.config/Dankestia/plugins/YourPlugin/
...
- **Provides searchable/actionable items?** - Use `launcher`. Items appear in the DANKESTIA launcher
...
Item {
    ...
    function executeItem(item) {
        ...
        else if (type === "copy") Quickshell.execDetached(["dankestia", "cl", "copy", data])
    }
}
```
...
Quickshell.execDetached(["dankestia", "cl", "copy", textToCopy])
```
...
2. Run the shell with verbose output: `qs -v -p $CONFIGPATH/quickshell/dankestia/shell.qml`
...
5. **Using `globalThis.clipboard`** - Does not exist; use `Quickshell.execDetached(["dankestia", "cl", "copy", text])`
