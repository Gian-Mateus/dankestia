...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Manage DANKESTIA authentication sync",
	Long:  "Manage shared PAM/authentication setup for DANKESTIA greeter and lock screen",
}

var authSyncCmd = &cobra.Command{
	Use:     "sync",
	Short:   "Sync DANKESTIA authentication configuration",
	Long:    "Apply shared PAM/authentication changes for the lock screen and greeter based on current DANKESTIA settings",
...
func syncAuth(nonInteractive bool) error {
	if !nonInteractive {
		fmt.Println("=== DANKESTIA Authentication Sync ===")
		fmt.Println()
	}
...
func syncAuthInTerminal(nonInteractive bool) error {
	...
	shellSyncCmd := "dankestia auth sync"
	if len(syncFlags) > 0 {
