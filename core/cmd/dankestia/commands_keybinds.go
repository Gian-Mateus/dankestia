...
import (
	...
	"github.com/AvengeMedia/Dankestia/core/internal/keybinds/providers"
...
var keybindsRemoveCmd = &cobra.Command{
	...
	Long:  "Remove a keybind. For Hyprland this writes a negative override to dankestia/binds-user.lua so the key stays unbound across DANKESTIA updates. For other providers it deletes the entry from the managed file.",
...
var keybindsResetCmd = &cobra.Command{
	...
	Short: "Reset a keybind override to its DANKESTIA default",
	Long:  "Drop the user override for the given key so the DANKESTIA default re-applies. For providers without a separate default file (Niri, MangoWC) this is equivalent to remove.",
	Args:  cobra.ExactArgs(2),
