...
import (
	...
	"github.com/AvengeMedia/Dankestia/core/internal/server"
...
var runCmd = &cobra.Command{
	Use:     "run",
	Short:   "Launch quickshell with DANKESTIA configuration",
	Long:    "Launch quickshell with DANKESTIA configuration (qs -c dankestia)",
	PreRunE: findConfig,
	Run: func(cmd *cobra.Command, args []string) {
		daemon, _ := cmd.Flags().GetBool("daemon")
		session, _ := cmd.Flags().GetBool("session")
		if v, _ := cmd.Flags().GetString("log-level"); v != "" {
			if err := os.Setenv("DANKESTIA_LOG_LEVEL", v); err != nil {
				log.Fatalf("Failed to set DANKESTIA_LOG_LEVEL: %v", err)
			}
		}
		if v, _ := cmd.Flags().GetString("log-file"); v != "" {
			if err := os.Setenv("DANKESTIA_LOG_FILE", v); err != nil {
				log.Fatalf("Failed to set DANKESTIA_LOG_FILE: %v", err)
			}
		}
...
var restartCmd = &cobra.Command{
	Use:     "restart",
	Short:   "Restart quickshell with DANKESTIA configuration",
	Long:    "Kill existing DANKESTIA shell processes and restart quickshell with DANKESTIA configuration",
	PreRunE: findConfig,
	Run: func(cmd *cobra.Command, args []string) {
		restartShell()
	},
...
var killCmd = &cobra.Command{
	Use:   "kill",
	Short: "Kill running DANKESTIA shell processes",
	Long:  "Kill all running quickshell processes with DANKESTIA configuration",
	Run: func(cmd *cobra.Command, args []string) {
		killShell()
	},
}
...
var ipcCmd = &cobra.Command{
	...
	Short: "Send IPC commands to running DANKESTIA shell",
...
var debugSrvCmd = &cobra.Command{
	...
	Long:  "Start the Unix socket debug server for DANKESTIA",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsCmd = &cobra.Command{
	Use:   "plugins",
	Short: "Manage DANKESTIA plugins",
	Long:  "Browse and manage DANKESTIA plugins from the registry",
}

var pluginsBrowseCmd = &cobra.Command{
	Use:   "browse",
	Short: "Browse available plugins",
	Long:  "Browse available plugins from the DANKESTIA plugin registry",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsListCmd = &cobra.Command{
	...
	Long:  "List all installed DANKESTIA plugins",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsInstallCmd = &cobra.Command{
	...
	Long:  "Install a DANKESTIA plugin from the registry using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
...
var pluginsUninstallCmd = &cobra.Command{
	...
	Long:  "Uninstall a DANKESTIA plugin using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
			return nil, cobra.ShellCompDirectiveNoFileComp
...
var pluginsUpdateCmd = &cobra.Command{
	...
	Long:  "Update an installed DANKESTIA plugin using its ID (e.g., 'myPlugin'). Plugin names are also supported.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
			return nil, cobra.ShellCompDirectiveNoFileComp
...
// Git builds: dankestia (git) v0.6.2-XXXX
// Stable releases: dankestia v0.6.2
func formatVersion(version string) string {
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("dankestia (git) v%s-%s", matches[1], matches[2])
	}
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		...
		return fmt.Sprintf("dankestia (git) v%s-%s", baseVersion, matches[1])
	}

	// Stable release format: 0.6.2
	re = regexp.MustCompile(`^([\d.]+)$`)
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("dankestia v%s", matches[1])
	}

	return fmt.Sprintf("dankestia %s", version)
}

func getBaseVersion() string {
	paths := []string{
		"/usr/share/quickshell/dankestia/VERSION",
		"/usr/local/share/quickshell/dankestia/VERSION",
		"/etc/xdg/quickshell/dankestia/VERSION",
	}
