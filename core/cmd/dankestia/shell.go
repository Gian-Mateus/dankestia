...
func appendLogEnv(env []string) []string {
	if v := os.Getenv("DANKESTIA_LOG_LEVEL"); v != "" {
		env = append(env, "DANKESTIA_LOG_LEVEL="+v)
	}
	if v := os.Getenv("DANKESTIA_LOG_FILE"); v != "" {
		env = append(env, "DANKESTIA_LOG_FILE="+v)
	}
	return env
}
...
func runShellInteractive(session bool) {
	...
	go printASCII()
	fmt.Fprintf(os.Stderr, "dankestia %s\n", Version)
	...
	cmd.Env = append(os.Environ(), "DANKESTIA_SOCKET="+socketPath)
	...
	if isSessionManaged && hasSystemdRun() {
		cmd.Env = append(cmd.Env, "DANKESTIA_DEFAULT_LAUNCH_PREFIX=systemd-run --user --scope")
	}

	homeDir, err := os.UserHomeDir()
	if err == nil && os.Getenv("DANKESTIA_DISABLE_HOT_RELOAD") == "" {
		if !strings.HasPrefix(configPath, homeDir) {
			cmd.Env = append(cmd.Env, "DANKESTIA_DISABLE_HOT_RELOAD=1")
		}
	}
...
func restartShell() {
	pids := getAllDANKESTIAPIDs()

	if len(pids) == 0 {
		log.Info("No running DANKESTIA shell instances found. Starting daemon...")
		runShellDaemon(false)
		return
	}
	...
	for pid := range uniquePids {
		...
		if err := proc.Signal(syscall.SIGUSR1); err != nil {
			...
			log.Infof("Sent SIGUSR1 to DANKESTIA process with PID %d", pid)
		}
	}
}
...
func killShell() {
	...
	if len(pids) == 0 {
		log.Info("No running DANKESTIA shell instances found.")
		return
	}
	...
	for pid := range uniquePids {
		...
		if err := proc.Kill(); err != nil {
			...
			log.Infof("Killed DANKESTIA process with PID %d", pid)
		}
	}
...
func runShellDaemon(session bool) {
	...
	if !isDaemonChild {
		...
		log.Infof("DANKESTIA shell daemon started (PID: %d)", cmd.Process.Pid)
		return
	}
	...
	cmd.Env = append(cmd.Env, "QS_APP_ID=com.danklinux.dankestia")
...
func getFirstDANKESTIAPID() (int, bool) {
	dir := getRuntimeDir()
	entries, err := os.ReadDir(dir)
	if err != nil {
		return 0, false
...
func runShellIPCCommand(args []string) {
	...
	switch pid, ok := getFirstDANKESTIAPID(); {
	case ok:
...
func printIPCHelp() {
	fmt.Println("Usage: dankestia ipc <target> <function> [args...]")
	fmt.Println()
	...
	if err != nil {
		fmt.Println("Could not retrieve available IPC targets (is DANKESTIA running?)")
		return
	}
...
func logStartupFailure(startTime time.Time, exitCode int, tracker *stderrTracker) {
	...
	if containsFontCrashSignature(tracker.String()) {
		log.Errorf("DANKESTIA startup failed due to a potential font/rendering crash. Try running 'fc-cache -fv' and restarting DANKESTIA.")
	} else {
		log.Errorf("DANKESTIA startup failed (exit code %d). Run 'dankestia doctor' for more diagnostics.", exitCode)
	}
}
