...
var setupCmd = &cobra.Command{
	Use:               "setup",
	Short:             "Deploy DANKESTIA configurations",
...
type dankestiaConfigSpec struct {
	niriFile     string
	hyprFile     string
	mangoFile    string
...
var dankestiaConfigSpecs = map[string]dankestiaConfigSpec{
	"binds": {
		niriFile:  "binds.kdl",
		hyprFile:  "binds.lua",
		...
		hyprContent: func(t string) string {
			return strings.ReplaceAll(config.DANKESTIABindsLuaConfig, "{{TERMINAL_COMMAND}}", t)
		},
	...
	"layout": {
		...
		hyprContent:  func(_ string) string { return config.DANKESTIALayoutLuaConfig },
	...
	"colors": {
		...
		hyprContent:  func(_ string) string { return config.DANKESTIAColorsLuaConfig },
	...
	"outputs": {
		...
		hyprContent:  func(_ string) string { return config.DANKESTIAOutputsLuaConfig },
	...
	"cursor": {
		...
		niriContent:  func(_ string) string { return "" },
		hyprContent:  func(_ string) string { return config.DANKESTIACursorLuaConfig },
		mangoContent: func(_ string) string { return "" },
	},
	"windowrules": {
		...
		niriContent:  func(_ string) string { return "" },
		hyprContent:  func(_ string) string { return config.DANKESTIAWindowRulesLuaConfig },
		mangoContent: func(_ string) string { return "" },
	},
}
...
func runSetupDmsConfig(name string) error {
	spec, ok := dankestiaConfigSpecs[name]
	if !ok {
	...
	var dankestiaDir string
	switch compositor {
	case "niri":
		dankestiaDir = filepath.Join(utils.XDGConfigHome(), "niri", "dankestia")
	case "hyprland":
		dankestiaDir = filepath.Join(utils.XDGConfigHome(), "hypr", "dankestia")
	case "mango", "mangowc":
		dankestiaDir = filepath.Join(utils.XDGConfigHome(), "mango", "dankestia")
	}
	...
	path := filepath.Join(dankestiaDir, filename)
...
func runSetup() error {
	fmt.Println("=== DANKESTIA Configuration Setup ===")
