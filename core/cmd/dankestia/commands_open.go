...
var openCmd = &cobra.Command{
	...
	Long: `Open a target (URL, file, or other resource) using the DANKESTIA application picker.
...
Examples:
  dankestia open https://example.com                    # Open URL with browser picker
  dankestia open file.pdf                               # Open file (MIME auto-detected)
  dankestia open file.pdf --mime application/pdf        # Override MIME detection
  dankestia open document.odt --category Office         # Open with office applications`,
...
func runOpen(target string) {
	...
	case isScheme(target, "http://"), isScheme(target, "https://"), isScheme(target, "dankestia://"):
	...
	if detectedMimeType == "" && len(openCategories) == 0 && (isScheme(target, "http://") || isScheme(target, "https://") || isScheme(target, "dankestia://")) {
		method = "browser.open"
		params["url"] = target
	}
	...
	if err := sendServerRequestFireAndForget(req); err != nil {
		fmt.Println("DANKESTIA is not running. Please start DANKESTIA first.")
		os.Exit(1)
	}
