...
import (
	...
	"github.com/AvengeMedia/Dankestia/core/internal/blur"
	"github.com/AvengeMedia/Dankestia/core/internal/clipboard"
...
var doctorCmd = &cobra.Command{
	Use:   "doctor",
	Short: "Diagnose DANKESTIA installation and dependencies",
	Long:  "Check system health, verify dependencies, and diagnose configuration issues for DANKESTIA",
	Run:   runDoctor,
}
...
func checkSystemInfo() []checkResult {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "Unsupported distribution") {
			...
			case osRelease["PRETTY_NAME"] != "":
				message = fmt.Sprintf("%s (not supported by dankestia setup)", osRelease["PRETTY_NAME"])
				details = "DANKESTIA may work but automatic installation is not available"
			}
		}
	...
	case xdgSessionType == "x11":
		results = append(results, checkResult{catSystem, "Display Server", statusError, "X11 (DANKESTIA requires Wayland)", "", doctorDocsURL + "#display-server"})
	default:
		results = append(results, checkResult{
...
func checkVersions(qsMissingFeatures bool) []checkResult {
	dankestiaCliPath, _ := os.Executable()
	dankestiaCliDetails := ""
	if doctorVerbose {
		dankestiaCliDetails = dankestiaCliPath
	}

	results := []checkResult{
		{catVersions, "DANKESTIA CLI", statusOK, formatVersion(Version), dankestiaCliDetails, doctorDocsURL + "#dankestia-cli"},
	}
	...
	dankestiaVersion, dankestiaPath := getDANKESTIAShellVersion()
	if dankestiaVersion != "" {
		results = append(results, checkResult{catVersions, "DANKESTIA Shell", statusOK, dankestiaVersion, dankestiaPath, doctorDocsURL + "#dankestia-shell"})
	} else {
		results = append(results, checkResult{catVersions, "DANKESTIA Shell", statusError, "Not installed or not detected", "Run 'dankestia setup' to install", doctorDocsURL + "#dankestia-shell"})
	}

	return results
}

func getDANKESTIAShellVersion() (version, path string) {
	...
	if dankestiaPath, err := config.LocateDANKESTIAConfig(); err == nil {
		versionFile := filepath.Join(dankestiaPath, "VERSION")
		if data, err := os.ReadFile(versionFile); err == nil {
			return strings.TrimSpace(string(data)), dankestiaPath
		}
		return "installed", dankestiaPath
	}
...
func checkDANKESTIAInstallation() []checkResult {
	var results []checkResult

	dankestiaPath := ""
	if err := findConfig(nil, nil); err == nil && configPath != "" {
		dankestiaPath = configPath
	} else if path, err := config.LocateDANKESTIAConfig(); err == nil {
		dankestiaPath = path
	}

	if dankestiaPath == "" {
		return []checkResult{{catInstallation, "DANKESTIA Configuration", statusError, "Not found", "shell.qml not found in any config path", doctorDocsURL + "#dankestia-configuration"}}
	}

	results = append(results, checkResult{catInstallation, "DANKESTIA Configuration", statusOK, "Found", dankestiaPath, doctorDocsURL + "#dankestia-configuration"})

	shellQml := filepath.Join(dankestiaPath, "shell.qml")
	if _, err := os.Stat(shellQml); err != nil {
		results = append(results, checkResult{catInstallation, "shell.qml", statusError, "Missing", shellQml, doctorDocsURL + "#dankestia-configuration"})
	} else {
		results = append(results, checkResult{catInstallation, "shell.qml", statusOK, "Present", shellQml, doctorDocsURL + "#dankestia-configuration"})
	}

	if doctorVerbose {
		installType := "Unknown"
		switch {
		case strings.Contains(dankestiaPath, "/nix/store"):
			installType = "Nix store"
		case strings.Contains(dankestiaPath, ".local/share") || strings.Contains(dankestiaPath, "/usr/share"):
			installType = "System package"
		case strings.Contains(dankestiaPath, ".config"):
			installType = "User config"
		}
		results = append(results, checkResult{catInstallation, "Install Type", statusInfo, installType, dankestiaPath, doctorDocsURL + "#dankestia-configuration"})
	}
...
func checkConfigurationFiles() []checkResult {
	...
	dankestiaDir := "Dankestia"

	configFiles := []struct{ name, path string }{
		{"settings.json", filepath.Join(configDir, dankestiaDir, "settings.json")},
		{"clsettings.json", filepath.Join(configDir, dankestiaDir, "clsettings.json")},
		{"plugin_settings.json", filepath.Join(configDir, dankestiaDir, "plugin_settings.json")},
		{"session.json", filepath.Join(utils.XDGStateHome(), dankestiaDir, "session.json")},
		{"dankestia-colors.json", filepath.Join(cacheDir, dankestiaDir, "dankestia-colors.json")},
	}
...
func checkSystemdServices() []checkResult {
	...
	dankestiaState := getServiceState("dankestia", true)
	if !dankestiaState.exists {
		results = append(results, checkResult{catServices, "dankestia.service", statusInfo, "Not installed", "Optional user service", doctorDocsURL + "#services"})
	} else {
		status, message := statusOK, dankestiaState.enabled
		if dankestiaState.active != "" {
			message = fmt.Sprintf("%s, %s", dankestiaState.enabled, dankestiaState.active)
		}
		switch {
		case dankestiaState.active == "failed":
			status = statusError
		case dankestiaState.active == "active":
		case dankestiaState.enabled == "disabled":
			status, message = statusWarn, "Disabled"
		case dankestiaState.active == "inactive":
			status = statusError
		}
		results = append(results, checkResult{catServices, "dankestia.service", status, message, "", doctorDocsURL + "#services"})
	}
...
func formatResultsPlain(results []checkResult) string {
	...
	sb.WriteString("## DANKESTIA Doctor Report\n\n")
...
func checkFonts() []checkResult {
	...
	settingsPath := filepath.Join(configDir, "Dankestia", "settings.json")
