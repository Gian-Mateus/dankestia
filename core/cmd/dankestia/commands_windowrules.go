...
import (
	...
	"github.com/AvengeMedia/Dankestia/core/internal/windowrules/providers"
...
var windowrulesListCmd = &cobra.Command{
	...
	Long:  "List all window rules from compositor config file. Returns JSON with rules and DANKESTIA status.",
...
var windowrulesAddCmd = &cobra.Command{
	...
	Short: "Add a window rule to DANKESTIA file",
	Long:  "Add a new window rule to the DANKESTIA-managed rules file.",
...
var windowrulesUpdateCmd = &cobra.Command{
	...
	Short: "Update a window rule in DANKESTIA file",
	Long:  "Update an existing window rule in the DANKESTIA-managed rules file.",
...
var windowrulesRemoveCmd = &cobra.Command{
	...
	Short: "Remove a window rule from DANKESTIA file",
	Long:  "Remove a window rule from the DANKESTIA-managed rules file.",
	Args:  cobra.ExactArgs(2),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) == 0 {
			return []string{"hyprland", "niri", "mango"}, cobra.ShellCompDirectiveNoFileComp
...
var windowrulesReorderCmd = &cobra.Command{
	...
	Short: "Reorder window rules in DANKESTIA file",
...
type WindowRulesListResult struct {
	...
	DANKESTIAStatus *windowrules.DANKESTIARulesStatus `json:"dankestiaStatus,omitempty"`
}
...
func runWindowrulesList(cmd *cobra.Command, args []string) {
	...
	case "niri":
		...
		dankestiaRulesPath := provider.GetOverridePath()
		dankestiaRules, _ := provider.LoadDANKESTIARules()

		dankestiaRuleMap := make(map[int]windowrules.WindowRule)
		for i, dr := range dankestiaRules {
			dankestiaRuleMap[i] = dr
		}

		dankestiaIdx := 0
		for i, r := range allRules {
			if r.Source == dankestiaRulesPath {
				if dmr, ok := dankestiaRuleMap[dankestiaIdx]; ok {
		...
		result.DANKESTIAStatus = parseResult.DANKESTIAStatus
	...
	case "mango", "mangowc":
		...
		dankestiaIdx := 0
		for i, r := range allRules {
			if r.Source == "dankestia/windowrules.conf" {
				if dmr, ok := dankestiaRuleMap[dankestiaIdx]; ok {
					allRules[i].ID = dmr.ID
					allRules[i].Name = dmr.Name
				}
