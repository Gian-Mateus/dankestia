...
var systemCmd = &cobra.Command{
	...
	Long:  "System-level operations (updates, etc.). Runs against installed package managers directly; does not require the DANKESTIA server.",
}
...
Examples:
  dankestia system update --check                  # list available updates
  dankestia system update                          # apply updates (interactive prompt)
  dankestia system update --noconfirm              # apply updates without prompting
  dankestia system update --dry                    # simulate without changing anything
  dankestia system update --no-flatpak --noconfirm # apply system updates only
  dankestia system update --interval 3600          # set the server poll interval to 1h`,
	Run: runSystemUpdate,
}
...
func init() {
	...
	systemUpdateCmd.Flags().IntVar(&sysUpdateIntervalS, "interval", -1, "Set the DANKESTIA server poll interval in seconds and exit (requires running server)")
...
func runSystemUpdateSetInterval(seconds int) {
	...
	if err != nil {
		log.Fatalf("Failed: %v (is dankestia server running?)", err)
	}
	if resp.Error != "" {
