...
var rootCmd = &cobra.Command{
	Use:   "dankestia",
	Short: "dankestia CLI",
	Long:  "dankestia is the Dankestia management CLI and backend server.",
}

func init() {
	rootCmd.PersistentFlags().StringVarP(&customConfigPath, "config", "c", "", "Specify a custom path to the DANKESTIA config directory")
}
...
func findConfig(cmd *cobra.Command, args []string) error {
	...
	var err error
	configPath, err = config.LocateDANKESTIAConfig()
	if err != nil {
		return err
	}
