...
func TestSyncGreeterConfigsAndAuthDelegatesSharedAuth(t *testing.T) {
	...
	var calls []string
	greeterConfigSyncFn = func(dankestiaPath, compositor string, logFunc func(string), sudoPassword string) error {
		if dankestiaPath != "/tmp/dankestia" {
			t.Fatalf("unexpected dankestiaPath %q", dankestiaPath)
		}
		if compositor != "niri" {
	...
	err := syncGreeterConfigsAndAuth("/tmp/dankestia", "niri", func(string) {}, sharedpam.SyncAuthOptions{
...
func TestSyncGreeterConfigsAndAuthStopsOnConfigError(t *testing.T) {
	...
	err := syncGreeterConfigsAndAuth("/tmp/dankestia", "niri", func(string) {}, sharedpam.SyncAuthOptions{}, nil)
