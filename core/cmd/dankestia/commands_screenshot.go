...
import (
	...
	"github.com/AvengeMedia/Dankestia/core/internal/screenshot"
...
Examples:
  dankestia screenshot                     # Region select, save file + clipboard
  dankestia screenshot full                # Full screen of focused output
  dankestia screenshot all                 # All screens combined
  dankestia screenshot output -o DP-1      # Specific output
  dankestia screenshot window              # Focused window (Hyprland)
  dankestia screenshot last                # Last region (pre-selected)
  dankestia screenshot --reset             # Reset last region pre-selection
  dankestia screenshot --no-clipboard      # Save file only
  dankestia screenshot --no-file           # Clipboard only
  dankestia screenshot --no-confirm        # Region capture on mouse release
  dankestia screenshot --cursor=on         # Include cursor
  dankestia screenshot -f jpg -q 85        # JPEG with quality 85`,
}
...
func setPopoutScreenshotMode(begin bool) {
	...
	cmdArgs := []string{"ipc"}
	if pid, ok := getFirstDANKESTIAPID(); ok {
