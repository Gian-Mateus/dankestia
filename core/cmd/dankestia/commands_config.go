...
func checkHyprlandInclude(filename string) (IncludeResult, error) {
	...
	targetPath := filepath.Join(configDir, "dankestia", filename)
	...
	targetRel := filepath.ToSlash(filepath.Join("dankestia", filename))
...
func checkNiriInclude(filename string) (IncludeResult, error) {
	...
	processed := make(map[string]bool)
	result.Included = niriFindInclude(mainConfig, "dankestia/"+filename, processed)
	return result, nil
}
...
func checkMangoWCInclude(filename string) (IncludeResult, error) {
	...
	processed := make(map[string]bool)
	result.Included = mangowcFindInclude(mainConfig, "dankestia/"+filename, processed)
	return result, nil
}
