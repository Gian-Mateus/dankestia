...
func GetQtLoggingRules() string {
	level := os.Getenv("DANKESTIA_LOG_LEVEL")
	if level == "" {
		level = "info"
	}
...
func GetLogger() *Logger {
	initLogger.Do(func() {
		...
		if envLevel := os.Getenv("DANKESTIA_LOG_LEVEL"); envLevel != "" {
		...
		if path := os.Getenv("DANKESTIA_LOG_FILE"); path != "" {
			_ = SetLogFile(path)
		}
	})
	return logger
...
// DANKESTIA_LOG_LEVEL. Unknown values default to info.
...
// ApplyEnvOverrides re-reads DANKESTIA_LOG_LEVEL and DANKESTIA_LOG_FILE and reconfigures
// the singleton. Safe to call after CLI flags have rewritten the environment.
func ApplyEnvOverrides() {
	GetLogger()
	if level := os.Getenv("DANKESTIA_LOG_LEVEL"); level != "" {
		SetLevel(level)
	}
	if path := os.Getenv("DANKESTIA_LOG_FILE"); path != "" {
