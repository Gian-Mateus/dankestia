...
func LocateDANKESTIAConfig() (string, error) {
	var primaryPaths []string

	configHome, err := os.UserConfigDir()
	if err == nil && configHome != "" {
		primaryPaths = append(primaryPaths, filepath.Join(configHome, "quickshell", "dankestia"))
	}
	...
	for dir := range strings.SplitSeq(dataDirs, ":") {
		if dir != "" {
			primaryPaths = append(primaryPaths, filepath.Join(dir, "quickshell", "dankestia"))
		}
	}
	...
	return "", fmt.Errorf("could not find DANKESTIA config (shell.qml) in any valid config path")
}
